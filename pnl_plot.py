...
def cummulative_pnls(pnls):
    size = len(pnls)
    for i in range(1, size): pnls[i] = pnls[i-1] + pnls[i]
    # return pnls
...
def merge_dates(all_dates):
    dates = []
    for list in all_dates: dates.extend(list)
    dates.sort()
    n = len(dates)
    m = 0
    for i in range(0, n):
...
def sampling_date(dates, s_size):
    size = len(dates)
    if (size <= s_size): return dates
    m_pos = [0]
    for i in range(1, size):
        if (dates[i] - dates[i-1] > 40): m_pos.append(i)
    m_size = len(m_pos)
    ...
    while (True):
        cc = 0
        for i in range(0, m_size-1): cc += (m_pos[i+1] - m_pos[i] - 1) // d
        r = s - cc
        if (r >= 0): break
        d += 1
    k = m_size-2
...
def check_sampling_date(dates):
    ...
    while (i < n):
        x = dates[i] // 100
        j = i+1
        while ((j < n) and (dates[j] // 100 == x)): j += 1
        print(dates[i], (j-i))
        i = j
...
def sampling_label_base(dates, max_size):
    ...
    indices = [0]
    for i in range(1, size):
        if ((dates[i] - dates[i-1]) > 40): indices.append(i)
    n = len(indices)
    if (n <= max_size): return indices
    m = 0
    for i in range(0, n):
        ...
        if (r == 1):
            ...
            m += 1
    # print(indices[0:m])
    if (m <= max_size): return indices[0:m]
    n = m
    m = 0
    for i in range(0, n):
        r = dates[indices[i]] // 100
...
def sampling_label(dates, max_size):
    ...
    if ((len(indices) > 1) and (indices[1] - indices[0] < len(dates) // (2*max_size))): del indices[0]
    return indices
...
def extract_plot_data(dates, a_dates, a_pnls):
    ...
    for i in range(0, len(dates)):
        while ((j < a_size) and (a_dates[j] < dates[i])): j += 1
...
show_labels = []
for index in show_indices: show_labels.append(dates[index])
colors = ["r", "g", "b", "c", "m", "y"]
fig, ax = plt.subplots()
for i in range(0, n):
